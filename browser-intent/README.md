...
- `place_order` — `{ site, symbol, side (buy|sell), quantity, limit_price, order_type? (DAY|GTC|ATC), board? (MAIN|ODD), dry_run?: true }`. Allowlisted on `col_financial`. Defaults to `dry_run: true`: fills the order form and clicks COL's native "Preview Order" button, returning the broker's Step-2-of-3 preview (computed fees, total amount, estimated commission) **without** placing an order. Set `dry_run: false` to ALSO click the post-preview confirm and place it for real — never do this without surfacing the dry-run preview to the user verbatim and getting explicit confirmation first. The worker classifies broker validation errors structurally: `reason: "board_lot_violation"` (qty below the symbol's MAIN-board lot — switch to `board: "ODD"` or raise qty), `insufficient_buying_power`, `insufficient_shares`, `symbol_not_in_portfolio` (sell-side), `tick_size_violation`, `invalid_symbol`, `market_closed` (PSE hours Mon-Fri 9:30 AM-12:00 PM and 1:00-3:15 PM PHT), `frameset_context_missing` (worker landed on the order form outside COL's frameset — recoverable by re-running after a fresh login). Use the `place_col_order` MCP prompt for the full guided workflow rather than calling this raw.
...
- `diagnose_order_form` — `{ site }`. Post-login. Locates trade/buy/sell nav links, attempts to activate one, then dumps every frame's form metadata (inputs, selects with option counts but NO option labels, buttons). Used to author `place_order` selectors against the real DOM.
- `diagnose_order_preview` — `{ site, symbol, quantity, side, limit_price, order_type?, board? }`. Post-login. Runs the `place_order` fill + Preview-Order-click flow with caller-supplied parameters and dumps the resulting preview-page DOM **without** clicking any confirm button. Used to capture confirm-page selectors when `place_order(dry_run=false)` returns `confirm_button_not_found`.
...
- `place_col_order` — guided write-op submission for `place_order`. Same dry-run-then-confirm contract as `submit_claim_from_receipt`. Surfaces structured broker reasons (`board_lot_violation`, `insufficient_buying_power`, `market_closed`, etc.) so the LLM can give the user an actionable next step instead of paraphrasing raw HTML. Args: `site`, `symbol`, `quantity`, `side`, `limit_price`, `order_type?`, `board?`. Currently scoped to `col_financial`.
