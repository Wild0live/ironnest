...
```
   ...
   Operator and governed-operation paths (outside the memory policy kernel):
       ...
       Traefik + Authelia
          ...
          │       ├──► default bridge ──► wiki-service     report publish + wiki session handoff
       ...
       hermes-pf-bigbert
          ├──► D:/LLM Wiki/wiki (published tree ro; proposal slice rw)
          └──► llm-wiki_wiki-net (external cross-stack network)
...
**Tier A control plane — Mission Control, Apps, governed operations, and Wiki publication.** Mission Control joins `platform-net` for Traefik and profile-bridge traffic plus the internal `mission-control-ops-net` for exact requests to the optional `operations-runner`. It never joins either memory network and never receives the Docker socket. `operations-runner` is the sole Docker-socket holder and accepts only allowlisted, persisted, single-use actions. Octo administration is a single operator-attributed lease opened with an operator-bound WebAuthn assertion, capped at ten minutes and two minutes idle, and limited to explicitly enrolled workloads; protected control-plane and Docker-socket containers remain excluded, while destructive actions require a fresh operation-specific approval. Registered WebAuthn transports are retained only as browser-picker hints (legacy security keys default to `usb`); origin, RP ID, signature, operator ownership, and user verification remain authoritative. Pending operations are surfaced on the requesting agent's avatar with a separate approval shield/count so health/work presence remains independently visible. Windows work crosses a separate localhost boundary through the host-bind queue; the default elevated host runner executes locally implemented remediation IDs rather than submitted PowerShell. Agent-authored web Apps are served read-only by `artifact-apps` on `apps.ironnest.local` under a restrictive CSP and separate origin. Reports are published to the LLM Wiki through the default profile bridge, which holds the Infisical-delivered wiki credential and can mint the scoped browser wiki session without exposing that credential to Mission Control.
...
**Tier A knowledge tooling — Big Bert and LLM Wiki.** Big Bert's dynamic service fragment joins the external `llm-wiki_wiki-net`, mounts `D:\LLM Wiki\wiki` read-only at `/knowledge/wiki`, and overlays only `_review/proposals` read-write. The wiki role installer is also mounted read-only and runs before the Hermes gateway. This cross-stack path is intentionally separate from OpenViking and does not grant Big Bert arbitrary writes across the wiki tree.
