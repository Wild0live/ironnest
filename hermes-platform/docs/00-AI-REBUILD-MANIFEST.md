...
9. **IronNest Task workflow** — Mission Control's governed layer over Hermes Kanban. A Task can start as a triage goal, be decomposed by the orchestrator, routed by profile role descriptions, executed by specialist agents such as Steve for code, Little John for security, `qa` for independent verification, and Octo for platform operations, then reviewed through logs, Reports, Apps, artifacts, and an approval-gated LLM Wiki publish path mediated by the default profile bridge.
...
13. **Big Bert wiki integration** — the `bigbert` profile mounts the published LLM Wiki read-only, mounts only the proposals subdirectory read-write, and joins the external `llm-wiki_wiki-net` for its direct wiki-channel role. This is a cross-stack knowledge boundary, not OpenViking memory and not a general host-write capability.
...
This bundle reconstructs **Hermes Platform** from the checked-out `platform/` repository. It does not independently recreate the wider IronNest perimeter: Rancher Desktop/WSL2, `platform-net`, `platform-egress`, AdGuard, Squid, Infisical, Traefik/Authelia, Wazuh, and the read-only `wazuh-query` broker are prerequisites created by `platform/bootstrap.sh` and their own stack definitions. The current Big Bert integration also requires the separate `D:\LLM Wiki` checkout, its external `llm-wiki_wiki-net`, the published wiki/proposals directories, and the two read-only role-installer files referenced by `services.d/hermes-pf-bigbert.yml`. A blank-Windows-host rebuild must establish those prerequisites first.
