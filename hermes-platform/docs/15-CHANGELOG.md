...
### Added
- **Sanitized live Agent Activity in Mission Control chat** now translates Hermes ACP plan and tool lifecycle events into a collapsible operator timeline while final-answer tokens continue streaming. Raw thought chunks, commands, tool inputs, tool outputs, prompts, and memory payloads are discarded inside the profile bridge; Mission Control applies a second allowlist, persists only a bounded per-turn milestone summary, and offers operator-scoped Compact, Standard, and Detailed display modes.
- **Automatic outage-only cron recovery** now records a durable Mission Control heartbeat and, after restart, asks every profile bridge to run at most the newest unattempted occurrence per schedule inside the exact offline window. First deployment does not replay historical stale jobs; failed runs and crash-after-claim records count as attempts; unreachable profiles remain queued for retry; and all executions continue through Hermes' existing cron, approval, and allowlist paths.
- **Conversation-bound approval acknowledgements** now persist idempotent request/executing/completed/failed lifecycle events in the exact originating Mission Control chat. FIDO receipt and terminal outcomes also trigger a best-effort natural acknowledgement from the requesting Hermes profile; deterministic control-plane events remain authoritative if the agent is busy or unavailable.
- **Little John software-remediation guidance** now ships as an authored skill for the fixed `software-vulnerability-remediation-v1` Winget lane, and the proposal helper rejects invented remediation IDs or missing structured payloads before an unusable FIDO request is created.
- **Operator rejection for pending Mission Control approvals** adds a red Reject action beside FIDO approval in Octo/Little John threads and the Approvals ledger. Rejection is an Authelia-authenticated, terminal deny-only transition that never contacts the Docker or Windows host runner, remains visibly attributed in chat/history, and cannot be approved afterward.
...
### Changed
- Mission Control refreshes the active, idle conversation while polling operation status, labels lifecycle records as agent operation updates, delays unbound-request fallback long enough to correlate cited `op-...` IDs, and uses a dedicated Approvals conversation instead of guessing the latest thread.
- FIDO-approved Docker lifecycle requests may now target explicitly Octo-enrolled existing workloads as well as `factory-*` containers. Exact target matching, protected names/labels, and Docker-socket-holder exclusions still apply; protected services such as `openclaw-infisical-agent` remain denied.
...
- Canonical memory-network wording now matches the deployed three-member backend network: Memory Gateway remains the sole application ingress to OpenViking, Ollama is the explicitly trusted co-resident embedding backend that OpenViking calls, and profiles/Mission Control retain no direct OpenViking route. The duplicate historical `D-011` ttyd decision entry was consolidated without dropping its deployment history.
